...
class ModelBundle:
    ...
    def __init__(self) -> None:
        self.dataset = pd.read_parquet(artifact_file("dataset.parquet"))
        self.spec = json.loads((artifact_file("feature_spec.json")).read_text(encoding="utf-8"))
        self.feature_cols = self.spec["FEATURE_COLS"]
        self.target_cols = self.spec["TARGET_COLS"]

        with open(artifact_file("selected_features.pkl"), "rb") as fh:
            self.selected_features = pickle.load(fh)
        with open(artifact_file("final_rf_models.pkl"), "rb") as fh:
            self.regressors = pickle.load(fh)
        with open(artifact_file("final_classifiers.pkl"), "rb") as fh:
            self.classifiers = pickle.load(fh)
        with open(artifact_file("final_hurdle_model.pkl"), "rb") as fh:

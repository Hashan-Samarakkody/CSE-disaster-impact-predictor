...
is scored independently and saved under its own name.

Each experiment is evaluated against the SAME chronological walk-forward folds and the
SAME purge rule as the main pipeline (`generate_walk_forward_splits` + `purge_horizon_overlap`).
Feature selection, SMOGN, and any regime-threshold learning happen on each fold's TRAINING
rows only -- never on the pooled test predictions, and never on the final held-out fold.

Scope note (ponytail): experiments 1-5 are each compared independently against the SAME
frozen baseline/config, not chained into one compounding pipeline -- answers "did X help"
for each X on its own, which is what section J's per-experiment record asks for, without
the added complexity and runtime risk of a sequential ratchet. A chained best-of-all-wins
config is reported at the end IF any experiment shows a real (bootstrap-CI-supported) gain.

Run after the main pipeline (needs `dataset.parquet`, `feature_spec.json`,
`results_regression.pkl`):

    python scripts/run_y1_experiments.py
"""
...
# Computed directly from the raw market panel, NOT persisted into feature_eng.py/dataset.parquet
# -- this keeps the experiment self-contained and avoids touching the shared pipeline the main
# notebook run depends on (per the "controlled experiment, not a silent pipeline replacement"
# instruction). All windows end at or before the event's own reference trading day (pos).

def build_regime_features(dataset):
    market = pd.read_parquet(ARTIFACTS / "market.parquet").sort_values("date").reset_index(drop=True)
...
def main():
    ...
    print("=" * 100)
    with open(ARTIFACTS / "results_regression.pkl", "rb") as f:
        import pickle
    ...
    stability.to_csv(ARTIFACTS / "y1_feature_stability.csv", index=False)
    ...
    print("=" * 100)
    n_folds_b = len(baseline_results["ensemble"][TARGET]["y_true"])
    ...
    print(table.to_string(index=False))
    table.to_csv(ARTIFACTS / "y1_experiments_ranked.csv", index=False)
    print(f"\nsaved: {ARTIFACTS / 'y1_experiments_ranked.csv'}, "
          f"{ARTIFACTS / 'y1_feature_stability.csv'}")

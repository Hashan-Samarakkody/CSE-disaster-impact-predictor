...
protocol (docs/METHODOLOGY_AUDIT.md, "Confirmation diagnostics" section).

No new hyperparameter/model search happens here. Y1 evaluates exactly two frozen
candidates (original ensemble, k=10 Ridge) with:
    A. per-fold stability
    B. prediction variance/calibration (+ actual-vs-predicted and residual plots)
    C. leave-one-observation-out influence (array-only, NO retraining)
    D. a written decision against the predefined hierarchy
Y2 runs the same A/B/C on its current best model (random_forest, collinearity-drop
features already live in the cache) vs naive_zero, and reports the pooled comparison
against the previously-printed pre-collinearity Y2 numbers (that run's per-fold arrays
no longer exist -- flagged, not fabricated).

Run after `scripts/run_y1_experiments.py` (needs `artifacts/results_regression.pkl`
for the frozen ensemble, and reuses `run_y1_experiments.py`'s own helpers for k=10 Ridge):

    python scripts/run_y1_y2_confirmation.py
"""
...
import run_y1_experiments as y1x  # reuses load_data, run_single_target, TARGET, etc.
...
def influence_analysis(y_true, y_pred, label):
    ...
    print(f"  top-5 most influential observations (index, actual, predicted, R2 w/o it, delta):")
    for i in order:

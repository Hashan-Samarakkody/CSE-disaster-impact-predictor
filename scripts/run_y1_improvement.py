"""Y1 improvement grid — everything pre-declared in docs/Y1_Y3_IMPROVEMENT_PREDECLARATION.md.

Ablations A1-A11: 4 horizons x 4 information sets x 3 feature capacities x 5 models,
scored on the SAME chronological walk-forward folds and the SAME test events, with each
horizon purged against its OWN label-end date (h=20 therefore embargoes 20 sessions).

Nothing on disk that the frozen pipeline owns is touched: the horizon targets are built
in memory from `artifacts/market.parquet` (`src/evaluation/y1_horizons.py`) rather than by
regenerating `dataset.parquet`, so `Y2_abnormal_volume` cannot move. `tests/test_y2_frozen.py`
proves it.

    python scripts/run_y1_improvement.py

Writes:
    artifacts/y1_improve_oof.parquet          one row per (config, test event): y_true, y_pred
    artifacts/y1_improve_metrics.parquet      one row per config: N, RMSE, MAE, pooled R2, skills
    artifacts/y1_improve_verdicts.parquet     paired episode-cluster bootstrap vs 3 baselines + Holm
    artifacts/y1_improve_stability.parquet    feature selection_frequency / mean_rank / median_rank
    artifacts/y1_improve_direction.parquet    secondary direction-classification analysis
"""
...
from src.evaluation.y1_horizons import (HORIZONS, build_horizon_targets, horizon_col,
...
def stage_a_expected_returns(market_feats, event_positions, horizons):
    ...
    The daily analogue of the event target: for reference session p, features are the
    market-state columns at p-1 (exactly the snapshot rule the event dataset uses --
    verified against `dataset.parquet`), and the label is 100*ln(P_{p+h}/P_{p-1}).
    Ridge on standardized features; no disaster variable ever enters this model.
    """
...
def main():
    ...
    market = pd.read_parquet(ART / "market.parquet")
    market_feats = pd.read_parquet(ART / "market_feats.parquet")
    feature_cols = json.loads((ART / "feature_spec.json").read_text())["FEATURE_COLS"]
    ...
    cache = ART / "y1_stage_a_expected.parquet"
    if cache.exists():
    ...
    for h in HORIZONS:
        ...
        for fold_i, split in enumerate(splits):
            ...
            for info in INFO_SETS:
                ...
                # expected-return estimate. Those rows are dropped from the residual
                # architecture's TRAINING side rather than costing the whole fold -- the
                # TEST side must be complete, otherwise this config would be scored on
                # different events from every other one and the paired comparison
                # (protocol 3.1) would be broken, so a missing test estimate skips the fold.
    ...
    oof.to_parquet(ART / "y1_improve_oof.parquet", index=False)
    print(f"\nwrote y1_improve_oof.parquet ({len(oof)} rows)")
    ...
    mt.to_parquet(ART / "y1_improve_metrics.parquet", index=False)
    ...
    if len(vt):
        ...
        vt["verdict"] = np.where(vt["boot_beats"], "A - statistically supported",
                                 np.where(vt["delta_rmse"] > 0, "B - suggestive but uncertain",
                                          "C - unsupported"))
    vt.to_parquet(ART / "y1_improve_verdicts.parquet", index=False)
    print(f"wrote y1_improve_metrics.parquet ({len(mt)} configs), "
          f"y1_improve_verdicts.parquet ({len(vt)} comparisons, "
    ...
    stab.to_parquet(ART / "y1_improve_stability.parquet", index=False)
    print(f"wrote y1_improve_stability.parquet ({len(stab)} rows)")

    # ----------------------------------------------------------- direction (secondary)
    direction = run_direction_analysis(data, event_dates, splits, sets["combined"])
    direction.to_parquet(ART / "y1_improve_direction.parquet", index=False)
    print(f"wrote y1_improve_direction.parquet ({len(direction)} rows)")

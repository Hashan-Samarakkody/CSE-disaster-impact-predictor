...
Pre-declared in docs/Y1_Y3_IMPROVEMENT_PREDECLARATION.md section 3. Same chronological
walk-forward folds and the same target-specific purge as the frozen pipeline; the target
itself (`Y3_recovery_days`, its 5-session adverse-response gate and its competing-event
censoring) is NOT touched -- only the likelihood it is fitted under, and how it is scored.

Primary training rows are GENUINE EVENTS ONLY: SMOGN interpolates a duration but cannot
generate a valid (duration, event_observed) pair, so a synthetic row asserts a recovery
that was never observed. B1 (existing point regression) and B6 (existing SMOGN on/off
ablation) are read from the already-cached artifacts, not re-run.

    python scripts/run_y3_improvement.py

Writes:
    artifacts/y3_improve_oof.parquet      per (model, test event): duration, censoring,
                                          predicted median, S(t) and P(T<=t) at 5 horizons
    artifacts/y3_improve_metrics.parquet  C-index (+ episode-clustered CI), IBS,
                                          uncensored point errors, per model
    artifacts/y3_improve_calibration.parquet  predicted vs Kaplan-Meier P(T<=t)
    artifacts/y3_improve_categories.parquet   the two pre-specified recovery categories
"""
...
def main():
    ...
    oof.to_parquet(ART / "y3_improve_oof.parquet", index=False)
    print(f"\nwrote y3_improve_oof.parquet ({len(oof)} rows, {oof.model.nunique()} models)")
    ...
    mt["verdict"] = np.where(mt["c_index_beats_chance"], "A - statistically supported",
                             np.where(mt["c_index"] > 0.5, "B - suggestive but uncertain",
                                      "C - unsupported"))
    mt.to_parquet(ART / "y3_improve_metrics.parquet", index=False)
    pd.concat(calib, ignore_index=True).to_parquet(ART / "y3_improve_calibration.parquet",
                                                    index=False)
    print(f"wrote y3_improve_metrics.parquet ({len(mt)} models), y3_improve_calibration.parquet")

    # ----------------------------------------------------------- recovery categories
    cat = recovery_categories(oof, episodes_all)
    cat.to_parquet(ART / "y3_improve_categories.parquet", index=False)
    print(f"wrote y3_improve_categories.parquet ({len(cat)} rows)")
...
def recovery_categories(oof, episodes_all, threshold=20):
    ...
    Category #2 is the existing, unchanged `C3b_slow_recovery` classifier, reported from
    `artifacts/classification_summary.parquet`.
    """

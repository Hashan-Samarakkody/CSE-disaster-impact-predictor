...
from src.evaluation.y1_horizons import (HORIZONS, MARKET_FEATURES, DISASTER_FEATURES,
...
def test_information_sets_partition_every_feature_column():
    ...
    spec = json.loads((ART / "feature_spec.json").read_text(encoding="utf-8"))
...
def test_real_market_series_produces_finite_targets_for_most_events():
    ...
    if not (ART / "market.parquet").exists():

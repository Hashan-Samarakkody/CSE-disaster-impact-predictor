...
The index-level study found that ASPI response to qualifying disasters is not predictable,
and its own explanation for that was aggregation: a national index absorbs a shock that
lands on one part of the economy. That explanation is testable. The same workbook that
supplies the ASPI carries 20 daily sector indices covering 2000-2023, so the same 64
events can be measured against Plantations, Hotels & Travels, Construction & Engineering
and seventeen others instead of against one blended number.

This is real data, not simulation: roughly 1,200 (event, sector) observations where the
index-level study had 64 events.

**Effective sample size is not 1,280.** Sectors co-move strongly within an event, so the
observations are clustered and the true information content sits somewhere between 64 and
1,280. Two consequences are enforced elsewhere and must not be dropped: folds split on
*events*, never inside one, and inference uses an event-level block bootstrap.

Y2 has no sector analogue -- the volume series is market-wide and cannot be decomposed by
sector -- so the panel carries Y1 and Y3 only.
"""

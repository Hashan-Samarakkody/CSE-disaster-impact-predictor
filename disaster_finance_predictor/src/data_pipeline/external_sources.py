...
Five sources, each reachability-tested before adoption, each cached to disk on first
fetch so that a re-run is offline and reproducible:

    DesInventar Sendai   district-level physical severity, 130,140 records, 1965-2020
    NASA POWER           daily precipitation and wind, 7 districts, 1999-2026
    FRED DEXSLUS         daily LKR/USD, 1973-2026
    countryeconomy.com   daily ASPI beyond the local archive's 2023-06-28 cutoff
    Wikidata             national election dates, for the confounder screen

Every feature built here is specified in `docs/EXTERNAL_DATA_PRE_DECLARATION.md`, which
was written before any of them was scored. The windows, thresholds and district points
in this module must match that document; changing one after seeing a score violates
standing integrity constraint 2.

The one thing this module deliberately does NOT do is recover monetary damage.
DesInventar Sri Lanka populates its USD loss field in 0 of 130,140 records. Block B is a
physical severity substitute and is named that way throughout.
"""
...
def extend_market_series(market: pd.DataFrame, extension: pd.DataFrame,
    ...
    Only rows strictly after the archive's last date are taken, so no existing value is
    ever overwritten by the external source -- the archive stays authoritative for the
    period it covers, and the join is verified by the overlap check in the self-test
    below (countryeconomy's 2023-06-28 close matches the archive's final row exactly).

    `trading_volume` is left NaN on the appended rows: countryeconomy publishes the
    index level only, and no practical post-2023 volume source was found. The existing
    per-target NaN mask drops these events from Y2, exactly as it already does for the
    2000 volume gap.
    """
    market = market.copy()

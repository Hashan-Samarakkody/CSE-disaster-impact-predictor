...
# figures.py -> evaluation[0] -> src[1] -> disaster_finance_predictor[2] -> repo root[3]
...
def _pretty(name: str) -> str:
    ...
    return (str(name)
            ...
            # log_return and lag_return_t-1 both used to render as "ret t-1", so the
            # heatmap showed two identically labelled rows. They are different columns:
            # at an event's as-of row, log_return is the previous session's return and
            # lag_return_t-1 is the one before that.
...
def plot_feature_correlation_heatmap(X, method: str = "spearman", cluster: bool = True,
    ...
    Spearman by default: `financial_damage` is zero for 47 of 64 events with a long tail
    on the rest, so Pearson there is driven by two or three points; and because Spearman
    is invariant to monotone transforms, a column and its own log read exactly 1.00, which
    is the cleanest possible statement that they are one variable.

    Cells below the n-dependent significance threshold are greyed out. At n=64 that
    threshold is |rho| ~ 0.246; without the mask a reader will point at a 0.3 cell and ask

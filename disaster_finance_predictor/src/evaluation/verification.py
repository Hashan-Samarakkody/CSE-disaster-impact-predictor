...
The results table answers "which RMSE is smaller". At 30 pooled out-of-fold test points
that is not the same question as "is this model better", and the study has so far only
ever answered the first. These helpers answer the second, using the stored out-of-fold
predictions -- no re-fitting required.

Two independent tests, because they fail in different ways at small n:

- `paired_bootstrap_delta` resamples the paired per-event errors. Distribution-free, but
  it assumes the events are exchangeable, which is a stretch for a time-ordered sample.
- `diebold_mariano` works on the loss differential series directly with a HAC variance.
  It is the standard forecast-comparison test, but its asymptotics are thin at n=30.

Agreement between them is meaningful. Disagreement means the sample cannot settle it,
